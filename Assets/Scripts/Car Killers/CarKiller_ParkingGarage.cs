...
public class CarKiller_ParkingGarage : MonoBehaviour
{
    ...
    bool front;
    [SerializeField] List<WheelCollider> w_colliders;
    ...
    IEnumerator SearchForUser()
    {
        ...
        if (!go)
        {
            if (targetDirection)
            {
                ...
                if ((Vector3.Distance(targetDirection.position, transform.position) > SpotDistance * 2) && drivingForward == null)
                {
                    Destroy(gameObject);
                    yield break;
                }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        sparkSet.transform.position = collision.transform.position;
